#!/usr/bin/env python3
"""
Generate agent prompt files from package agent-prompts templates.

Auto-discovers available agent types from package agent-prompts and generates
agent prompt files that can be used to dispatch subagents with full project context.

Generated by SpecifyX init command for claude.
"""
...
def discover_agent_types(helpers: ScriptHelpers) -> Tuple[bool, List[AgentType]]:
    """
    Auto-discover available agent types from package agent-prompts.
    ...
    try:
        ...
        # Use agent-prompts from package (the actual agent instructions)
        try:
            agent_prompts_dir = (
                importlib.resources.files(templates_pkg) / "agent-prompts"
            )
            if agent_prompts_dir.is_dir():
                for template_file in sorted(agent_prompts_dir.iterdir(), key=lambda p: p.name):
                    if template_file.is_file() and template_file.name.endswith(
                        ".md.j2"
                    ):
                        agent_name = template_file.name.replace(".md.j2", "")

                        # Try to read description from template frontmatter
                        try:
            ...
            echo_debug(f"Failed to discover from package agent-prompts: {str(e)}", True)
...
def get_project_context(helpers: ScriptHelpers) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use find_feature_directory_for_workflow to support both feature branches and main
        feature_dir = helpers.find_feature_directory_for_workflow()
        specs = {}
        plans = {}
        tasks = []
        ...
        if feature_dir and feature_dir.exists():
            ...
            if tasks_file.exists():
                ...
                # Extract task items and convert to structured objects
                task_counter = 1
                for line in tasks_content.split("\n"):
                    ...
                    if line.startswith("- [ ]") or line.startswith("- [x]"):
                        ...
                        # Parse task ID if present (e.g., "T001 Task description")
                        task_id_match = re.match(r"^(T\d+)\s+(.+)$", task_text)
                        if task_id_match:
...
def scaffold_agent(
    ...
    """
    try:
        # Use agent-prompts from package (not agent-templates)
        # agent-prompts are the actual agent instructions
        # agent-templates are report/output templates (not what we want)
        ...
        # Try to get template from package agent-prompts
        try:
            agent_prompts_dir = (
                importlib.resources.files(templates_pkg) / "agent-prompts"
            )
            template_path = agent_prompts_dir / agent_template_name

            if not template_path.is_file():
                return False, f"Agent prompt template not found: {agent_template_name}. Available agents: code-reviewer, documentation-reviewer, implementer, spec-reviewer, architecture-reviewer, test-reviewer"
        except Exception as e:
            return False, f"Failed to locate agent prompt templates: {str(e)}"
        ...
        # Render the template (convert Traversable to real filesystem Path)
        with importlib.resources.as_file(template_path) as real_template_path:
            success, error = helpers.render_template_standalone(
                template_path=real_template_path,
    ...
    try:
        ...
        if list_agents:
            if json_mode:
                ...
                if not agent_types:
                    echo_info(
                        "No agent prompts found in package resources (agent-prompts).", quiet
                    )
                    echo_info("Ensure your package includes agent-prompts or update to a version that does.", quiet)
                else:
